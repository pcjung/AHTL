								   ...
								   )
								{
										...
										*/
										...
										if(get_from_front1)
										{
												...
												last1 = from[idx1 + VLEN];
												idx1 += VLEN;
									  			_mm_prefetch(from + idx1 + VLEN*2, _MM_HINT_NT);
										}
										else
										{
												tmp1 = _MM_LOAD(from + idx2);
												last2 = from[idx2 + VLEN];
												idx2 += VLEN;
									  			_mm_prefetch(from + idx2 + VLEN*2, _MM_HINT_NT);
										}
										...
										if(get_from_front3)
										{
												...
												last3 = from[idx3 + VLEN];
												idx3 += VLEN;
									  			_mm_prefetch(from + idx3 + VLEN*2, _MM_HINT_NT);
										}
										else
										{
												tmp3 = _MM_LOAD(from + idx4);
												last4 = from[idx4 + VLEN];
												idx4 += VLEN;
									  			_mm_prefetch(from + idx4 + VLEN*2, _MM_HINT_NT);
										}
#ifndef NO_COMPUTE
										_VECTOR t1 = _MM_MIN(tmp1, tmp2);
										_VECTOR t2 = _MM_MAX(tmp1, tmp2);
