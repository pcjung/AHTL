...
/*
#ifdef BIN_SIMD
		for(i = start; i + 3 < count; i += 4)
		{
#ifdef SSE
			_VECTOR d1 = _MM_SET1(data[i]);
			_VECTOR d2 = _MM_SET1(data[i+1]);
			_VECTOR d3 = _MM_SET1(data[i+2]);
			_VECTOR d4 = _MM_SET1(data[i+3]);
			int i1, i2, i3, i4;
			i1=i2=i3=i4=0;
			for(j = 0; j < n2; j++)
			{
				_VECTOR cmp = v_boundary[j];
				_VECTOR c1 = _mm_cmpge_ps(d1, cmp); 
				_VECTOR c2 = _mm_cmpge_ps(d2, cmp); 
				_VECTOR c3 = _mm_cmpge_ps(d3, cmp); 
				_VECTOR c4 = _mm_cmpge_ps(d4, cmp); 
				int t1 = (_mm_movemask_ps(c1));
				int t2 = (_mm_movemask_ps(c2));
				int t3 = (_mm_movemask_ps(c3));
				int t4 = (_mm_movemask_ps(c4));
				i1 += _mm_popcnt_u32(t1);
				i2 += _mm_popcnt_u32(t2);
				i3 += _mm_popcnt_u32(t3);
				i4 += _mm_popcnt_u32(t4);
			}
			//printf("%f %f %f %f, %d %d %d %d\n", data[i], data[i+1], data[i+2], data[i+3], i1, i2, i3, i4);
			bin[i1]++;
			bin[i2]++;
			bin[i3]++;
			bin[i4]++;
