 ...
 )
{
		...
		int BLOCK_SIZE = N2bin_count * VLEN * BMCOEFF; // 4-way loop unrolled 
		...
		float * buffer1 = _mm_malloc(sizeof(float) * BLOCK_SIZE, 64);
		float * buffer2 = _mm_malloc(sizeof(float) * BLOCK_SIZE, 64);
		float * bufs[2];
		bufs[0] = buffer1;
		bufs[1] = buffer2;
		...
		for(iter_base = 0; iter_base < work_size; iter_base += BLOCK_SIZE){ // operate on 1 block
			...
			//	#define DEBUG_SORT
				for(step = 0, chunk_size=VLEN; chunk_size < VLEN * 4; step++, chunk_size *= 2)
				{
								//printf("%d\n", step);
...
#ifdef DEBUG_SORT
				printf("16wise sort coplete\n");
#endif
				for( chunk_size=VLEN * 4; chunk_size * 4 <= BLOCK_SIZE; step++, chunk_size *= 2)
				{
								//printf("%d\n", step);
						...
						for(
										chunk1 = 0, chunk2 = chunk1 + chunk_size ;
										chunk2 < BLOCK_SIZE; 
										chunk1 += chunk_size * 2, chunk2 = chunk1 + chunk_size 
						   )
						{
								int idx1= chunk1, idx2, out1;
								_VECTOR tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8;
								tmp5 = _MM_LOAD( from + idx1 );
								tmp6 = _MM_LOAD( from + idx1 + VLEN );
								tmp7 = _MM_LOAD( from + idx1 + VLEN * 2);
								tmp8 = _MM_LOAD( from + idx1 + VLEN * 3);

								float last1 = *(from + idx1 + VLEN * 4), last2;
								idx1 += VLEN * 4;
								int get_from_front1 = 0;

								for( 
												idx2 = chunk2, out1 = chunk1;
												out1 < chunk1 + chunk_size * 2 - VLEN * 4;
												out1 += VLEN * 4 
								   )
								{
										if(get_from_front1)
										{
												tmp1 = _MM_LOAD(from + idx1);
												tmp2 = _MM_LOAD(from + idx1 + VLEN);
												tmp3 = _MM_LOAD(from + idx1 + VLEN * 2);
												tmp4 = _MM_LOAD(from + idx1 + VLEN * 3);
												last1 = from[idx1 + VLEN * 4];
												idx1 += VLEN * 4;
												//_mm_prefetch((const char *)from + idx1, _MM_HINT_T0);
										}
										else
										{
												tmp1 = _MM_LOAD(from + idx2);
												tmp2 = _MM_LOAD(from + idx2 + VLEN);
												tmp3 = _MM_LOAD(from + idx2 + VLEN * 2);
												tmp4 = _MM_LOAD(from + idx2 + VLEN * 3);
												last2 = from[idx2 + VLEN * 4];
												idx2 += VLEN * 4;
												//_mm_prefetch((const char *)from + idx2, _MM_HINT_T0);
										}
										...
										tmp5 = _mm_shuffle_ps(tmp5, tmp5, 0x1b);
										tmp6 = _mm_shuffle_ps(tmp6, tmp6, 0x1b);
										tmp7 = _mm_shuffle_ps(tmp7, tmp7, 0x1b);
										tmp8 = _mm_shuffle_ps(tmp8, tmp8, 0x1b);
										...
										t1 = _MM_MIN(tmp1, tmp8);
										t5 = _MM_MAX(tmp1, tmp8);
										t2 = _MM_MIN(tmp2, tmp7);
										t6 = _MM_MAX(tmp2, tmp7);
										t3 = _MM_MIN(tmp3, tmp6);
										t7 = _MM_MAX(tmp3, tmp6);
										t4 = _MM_MIN(tmp4, tmp5);
										t8 = _MM_MAX(tmp4, tmp5);
...
#endif
										_MM_STORE(to + out1, tmp1);
										_MM_STORE(to + out1 + VLEN, tmp2);
										_MM_STORE(to + out1 + VLEN * 2, tmp3);
										_MM_STORE(to + out1 + VLEN * 3, tmp4);
								...
								printf("store at %d\n", out1);
#endif
								_mm_store_ps(to + out1, tmp5);
								_mm_store_ps(to + out1 + VLEN, tmp6);
								_mm_store_ps(to + out1 + VLEN * 2, tmp7);
								_mm_store_ps(to + out1 + VLEN * 3, tmp8);
						}
				}
#ifdef DEBUG_SORT
						   ...
						   )
						{
								   ...
								   )
								{
										...
										if(get_from_front2)
										{
												...
												last3 = from[idx3 + VLEN];
												idx3 += VLEN;
											  //_mm_prefetch((const char *)(from + idx3), _MM_HINT_T0);
										}
										else
										{
												...
												last4 = from[idx4 + VLEN];
												idx4 += VLEN;
											  //_mm_prefetch((const char *)(from + idx4), _MM_HINT_T0);
										}
										if(get_from_front3)
										{
												...
												last5 = from[idx5 + VLEN];
												idx5 += VLEN;
											  //_mm_prefetch((const char *)(from + idx5), _MM_HINT_T0);
										}
										else
										{
												...
												last6 = from[idx6 + VLEN];
												idx6 += VLEN;
											  //_mm_prefetch((const char *)(from + idx6), _MM_HINT_T0);
										}
										if(get_from_front4)
										{
												...
												last7 = from[idx7 + VLEN];
												idx7 += VLEN;
											  //_mm_prefetch((const char *)(from + idx7), _MM_HINT_T0);
										}
										else
										{
												...
												last8 = from[idx8 + VLEN];
												idx8 += VLEN;
											  //_mm_prefetch((const char *)(from + idx8), _MM_HINT_T0);
										}
