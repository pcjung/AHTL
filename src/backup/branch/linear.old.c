...
#pragma unroll(UNROLL_N)
			for(j = 1; j < n+1; j++)
			{
					__m128i t1 = _mm_castps_si128( _mm_cmp_ps(tmp1, v_boundary[j],_CMP_GE_OS));
					__m128i t2 = _mm_castps_si128( _mm_cmp_ps(tmp2, v_boundary[j],_CMP_GE_OS));
					__m128i t3 = _mm_castps_si128( _mm_cmp_ps(tmp3, v_boundary[j],_CMP_GE_OS));
					__m128i t4 = _mm_castps_si128( _mm_cmp_ps(tmp4, v_boundary[j],_CMP_GE_OS));
