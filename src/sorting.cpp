...
/****** Merge 2 block using SSE instruction set *******/
...
/****** Merge 2 block using AVX instruction set *******/
...
/****** Merge 2 block using MIC instruction set *******/
...
static int merge_4way(float ** bufs, int BLOCK_SIZE)
{
  ...
  for(step = 0, chunk_size=VLEN; chunk_size * 2 <= BLOCK_SIZE; step++, chunk_size *= 2)
  {
    ...
    for(
        ...
        chunk4 < BLOCK_SIZE;
        chunk1 += chunk_size * 4, chunk2 += chunk_size * 4,
        chunk3 += chunk_size * 4, chunk4 += chunk_size * 4
       )
    {
      ...
      {
        /*
           int x1 = (idx1 & (-get_from_front1)) + (idx2 & (~get_from_front1));
           int x2 = (idx3 & (-get_from_front3)) + (idx4 & (~get_from_front3));
           tmp1 = _MM_LOAD(from + x1);
           tmp3 = _MM_LOAD(from + x2);
           ...
           idx3 += VLEN & (-get_from_front3);
           idx4 += VLEN & (~get_from_front3);
           last1 = from[idx1];
           last2 = from[idx2];
           last3 = from[idx3];
           last4 = from[idx4];
        ...
        if(get_from_front1)
        {
          ...
          last1 = from[idx1 + VLEN];
          idx1 += VLEN;
          _mm_prefetch((const char *)(from + idx1 + VLEN*2), _MM_HINT_NT);
        }
        else
        {
          tmp1 = _MM_LOAD(from + idx2);
          last2 = from[idx2 + VLEN];
          idx2 += VLEN;
          _mm_prefetch((const char *)(from + idx2 + VLEN*2), _MM_HINT_NT);
        }
        ...
        if(get_from_front3)
        {
          ...
          last3 = from[idx3 + VLEN];
          idx3 += VLEN;
          _mm_prefetch((const char *)(from + idx3 + VLEN*2), _MM_HINT_NT);
        }
        else
        {
          tmp3 = _MM_LOAD(from + idx4);
          last4 = from[idx4 + VLEN];
          idx4 += VLEN;
          _mm_prefetch((const char *)(from + idx4 + VLEN*2), _MM_HINT_NT);
        }
#ifndef NO_COMPUTE
        _VECTOR t1 = _MM_MIN(tmp1, tmp2);
        _VECTOR t2 = _MM_MAX(tmp1, tmp2);
 ...
 )
{
  ...
  float * buffer1 = (float *)_mm_malloc(sizeof(float) * BLOCK_SIZE + VLEN, 64);
  float * buffer2 = (float *)_mm_malloc(sizeof(float) * BLOCK_SIZE + VLEN, 64);
  float * bufs[2]; 

  // use 2 buffer alternatively for each merge steps
  bufs[0] = buffer1;
  bufs[1] = buffer2;
...
#define imm2 0x01
    ...
    // Build histogram from sorted blocks.
